...
function App() {
  ...
  // 1. 🌟 HIỆU ỨNG SMOOTH SCROLL (LENIS VIA CDN)
  useEffect(() => {
    // Nhúng thư viện Lenis Scroll mượt mà thế hệ mới trực tiếp từ CDN
  ...
  // 2. 🌟 HIỆU ỨNG BACKGROUND MA TRẬN / SỐ HÓA (DIGITAL CANVAS BACKGROUND)
  useEffect(() => {
    ...
    // Tạo các điểm hạt lơ lửng kết nối với nhau (Mạng lưới Backend)
    const particles = [];
  ...
  // 3. 🌟 SPOTLIGHT TRACKING EFFECT (ÁNH SÁNG THEO CHUỘT)
  useEffect(() => {
  ...
  // DỮ LIỆU CV SẠCH 100% (ĐÃ BỎ CÁC CITE CHÚ THÍCH)
  const projects = [
    {
    ...
    <div className="bg-[#030712] text-slate-100 min-h-screen font-sans antialiased relative overflow-x-hidden selection:bg-teal-500 selection:text-slate-900">
      ...
      <header className="sticky top-0 z-50 backdrop-blur-xl bg-[#030712]/70 border-b border-slate-900 flex items-center justify-between px-6 h-20 transition-all">
        ...
        <nav className="flex space-x-8 text-xs uppercase tracking-widest font-semibold text-slate-400">
          {['Giới Thiệu', 'Kỹ Năng', 'Kinh Nghiệm', 'Dự Án'].map((item, i) => (
            <a 
              key={i} 
      ...
      {/* HERO SECTION - KIỂU DÁNG CÔNG NGHỆ HOÀNH TRÁNG */}
      <section id="about" className="relative z-10 max-w-6xl mx-auto px-4 pt-24 pb-20 md:pt-40 md:pb-32 flex flex-col md:flex-row items-center justify-between gap-16">
        <div className="max-w-2xl space-y-8 animate-fade-in">
          ...
          <p className="text-slate-400 leading-relaxed max-w-xl text-base md:text-lg border-l-2 border-slate-800 pl-4">
            Tốt nghiệp chuyên ngành Kỹ thuật Phần mềm, chuyên sâu về phát triển hệ thống Backend[cite: 7]. Có kinh nghiệm thực chiến trong việc xây dựng, tối ưu hóa các RESTful API hiệu năng cao bằng <span className="text-teal-400 font-bold">Node.js, NestJS, và TypeScript</span>[cite: 7]. Đã từng quản lý cơ sở dữ liệu quy mô lớn và tích hợp giải pháp lưu trữ hiện đại trong kỳ thực tập thực tế[cite: 8].
          </p>
          
          {/* THÔNG TIN LIÊN HỆ GỌN GÀNG, ĐẬM CHẤT CODE */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs font-mono text-slate-400 pt-2">
            <div className="flex items-center gap-3 bg-slate-900/30 px-4 py-3 rounded-xl border border-slate-800/60 hover:border-teal-500/30 hover:bg-slate-900/60 transition-all duration-300">
              <span className="text-teal-500">📥 EMAIL:</span> phqtquin3@gmail.com [cite: 2]
            </div>
            <div className="flex items-center gap-3 bg-slate-900/30 px-4 py-3 rounded-xl border border-slate-800/60 hover:border-teal-500/30 hover:bg-slate-900/60 transition-all duration-300">
              <span className="text-teal-500">📞 PHONE:</span> 0703 625 820 [cite: 2]
            </div>
            <div className="flex items-center gap-3 bg-slate-900/30 px-4 py-3 rounded-xl border border-slate-800/60 hover:border-teal-500/30 hover:bg-slate-900/60 transition-all duration-300 sm:col-span-2">
              <span className="text-teal-500">📍 LOCATION:</span> Quận 8, TP. Hồ Chí Minh [cite: 5]
            </div>
          </div>

          {/* NÚT CLICK HIỆU ỨNG HOVER GLASSMORPHISM */}
          <div className="flex gap-4 pt-4">
            <a href="https://github.com/DuckHau" target="_blank" rel="noreferrer" className="bg-slate-900 hover:bg-slate-800 text-white font-mono text-xs tracking-widest px-6 py-4 rounded-xl transition-all border border-slate-800 hover:border-slate-700 flex items-center gap-2 hover:scale-[1.02] active:scale-95 duration-300 shadow-xl">
              <span>🐙</span> GITHUB_PROFILE [cite: 6]
            </a>
        ...
        {/* KHỐI CÔNG NGHỆ PHÁT SÁNG NEON (GLOWING CORE) */}
        <div className="hidden md:block relative group">
          ...
          <div className="w-72 h-72 rounded-2xl bg-[#050b18]/95 border border-slate-800/80 flex flex-col items-center justify-center p-8 text-center space-y-6 shadow-2xl relative z-10 group-hover:border-teal-500/30 group-hover:scale-[1.03] transition-all duration-500">
            ...
            <div className="text-[11px] text-slate-400 font-mono space-y-1.5 w-full">
              <p className="bg-slate-900/80 py-1.5 rounded-lg border border-slate-800/50">Node.js • NestJS [cite: 11]</p>
              <p className="bg-slate-900/80 py-1.5 rounded-lg border border-slate-800/50">TypeScript [cite: 11]</p>
              <p className="bg-slate-900/80 py-1.5 rounded-lg border border-slate-800/50">MongoDB • SQL [cite: 11]</p>
            </div>
          </div>
        </div>
      </section>

      {/* CORE COMPETENCIES (SKILLS CHẠY HIỆU ỨNG TƯƠNG TÁC CAO) */}
      <section id="skills" className="relative z-10 max-w-6xl mx-auto px-4 py-24">
        <div className="text-center max-w-2xl mx-auto mb-20">
          <h2 className="text-4xl font-black tracking-tight mb-4 bg-gradient-to-r from-slate-100 to-slate-400 bg-clip-text text-transparent uppercase font-mono">Năng Lực Cốt Lõi [cite: 10]</h2>
          <div className="h-[2px] w-12 bg-gradient-to-r from-teal-500 to-cyan-500 mx-auto rounded-full"></div>
        </div>
        
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            { icon: "💻", title: "Ngôn Ngữ", desc: "TypeScript, JavaScript, C#, HTML/CSS, SQL [cite: 11]" },
            { icon: "⚙️", title: "Frameworks", desc: "Node.js, NestJS, Express.js, RESTful API [cite: 11]" },
            { icon: "🗄️", title: "Cơ Sở Dữ Liệu", desc: "MongoDB (Mongoose), SQL Server, MySQL (Tối ưu hóa Index) [cite: 11]" },
            { icon: "🛠️", title: "Công Cụ & Infra", desc: "Git/GitHub, Postman, Swagger, MinIO Storage, Figma, Jira [cite: 11]" }
          ].map((skill, index) => (
      ...
      {/* TIMELINE KINH NGHIỆM ĐẬM CHẤT CYBER */}
      <section id="experience" className="relative z-10 bg-[#040914]/40 border-y border-slate-900 py-24 backdrop-blur-sm">
        <div className="max-w-4xl mx-auto px-4">
          <div className="mb-16">
            <h2 className="text-4xl font-black tracking-tight mb-2 font-mono uppercase">💼 Kinh Nghiệm Làm Việc [cite: 12]</h2>
          </div>
          ...
          <div className="relative border-l border-slate-800 pl-8 ml-4 space-y-16">
            <div className="relative group">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div>
                  <h3 className="text-2xl font-black text-slate-200 group-hover:text-teal-400 transition-colors duration-300">Quoc Bao Company [cite: 13]</h3>
                  <p className="text-teal-400 font-mono font-semibold text-sm mt-1">Backend Developer Intern [cite: 13]</p>
                </div>
              ...
              <ul className="space-y-4 text-slate-400 text-sm list-none pl-0 font-mono">
                {[
                  "Thiết kế và phát triển các module cốt lõi Contact & Form bằng NestJS tạo ra RESTful API hiệu năng cao xử lý logic phức tạp[cite: 15].",
                  "Triển khai hạ tầng lưu trữ tập trung qua MinIO Object Storage & tối ưu hóa schema MongoDB giúp quản lý dữ liệu phi cấu trúc[cite: 16].",
                  "Tích hợp tài liệu tự động Swagger, chuẩn hóa dữ liệu request/response giúp giảm 25% chu kỳ dev-to-test[cite: 17].",
                  "Cải tiến cấu trúc mã nguồn và database index cấu hình giúp tối ưu hóa tốc độ truy vấn API và trải nghiệm người dùng[cite: 18]."
                ].map((item, idx) => (
      ...
      {/* KEY PROJECTS (HIỆU ỨNG THẺ CHUYỂN ĐỘNG SẮC NÉT) */}
      <section id="projects" className="relative z-10 max-w-6xl mx-auto px-4 py-24">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          <div>
            <h2 className="text-4xl font-black tracking-tight mb-2 font-mono uppercase">Dự Án Tiêu Biểu [cite: 19]</h2>
            <p className="text-slate-400 font-mono text-xs">Các kiến trúc hệ thống backend tôi đã xây dựng [cite: 22, 27, 31]</p>
          </div>
          
          {/* TAB LỌC THIẾT KẾ PHÁT SÁNG NHẸ */}
          <div className="flex bg-slate-950 p-1 rounded-xl border border-slate-900 self-start">
            {['all', 'backend', 'fullstack'].map((tab) => (
              ...
              >
                {tab === 'all' ? 'TẤT CẢ' : tab === 'backend' ? 'PURE_BACKEND' : 'FULL_STACK'}
              </button>
            ))}
          </div>
        </div>
              ...
              <div className="p-6 flex-1 flex flex-col justify-between space-y-6">
                ...
                <div className="pt-4 border-t border-slate-900 flex items-center justify-between">
                  <a href={project.github} target="_blank" rel="noreferrer" className="text-xs font-mono font-black text-teal-400 hover:text-teal-300 flex items-center gap-1.5 transition-colors uppercase tracking-wider">
                    <span>🐙</span> REPOSITORY_LINK ➔ [cite: 21, 26, 30]
                  </a>
                </div>
              </div>
            </div>
        ...
        <div className="max-w-4xl mx-auto px-4 grid md:grid-cols-2 gap-12">
          <div>
            <h3 className="text-xl font-mono font-black mb-6 text-slate-200 flex items-center gap-2 uppercase tracking-wider">
              🎓 Học Vấn & Chứng Chỉ [cite: 33, 35]
            </h3>
            <div className="space-y-4 font-mono">
              <div className="bg-slate-950 p-5 rounded-2xl border border-slate-900 hover:border-teal-500/10 transition-colors">
                <span className="text-xs font-bold text-teal-400">10/2021 - 07/2025 [cite: 36]</span>
                <h4 className="font-bold text-slate-200 mt-2 text-base">HUFLIT [cite: 33]</h4>
                <p className="text-sm text-slate-400 mt-1">Cử nhân Kỹ thuật Phần mềm [cite: 34]</p>
              </div>
              <div className="bg-slate-950 p-5 rounded-2xl border border-slate-900 hover:border-teal-500/10 transition-colors">
                <span className="text-xs font-bold text-teal-400">NĂM CẤP: 2024 [cite: 37]</span>
                <h4 className="font-bold text-slate-200 mt-2 text-base">TOEIC 620 International [cite: 35]</h4>
                <p className="text-sm text-slate-400 mt-1">Issued by IIG Vietnam [cite: 35]</p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="bg-slate-950 p-6 rounded-2xl border border-slate-900 h-[calc(100%-3.5rem)] flex items-center hover:border-teal-500/10 transition-colors">
              <p className="text-slate-400 text-sm font-mono leading-relaxed">
                Thành thạo trong việc khai thác tài liệu kỹ thuật bằng <strong className="text-teal-400 font-bold">Tiếng Anh</strong>, ghi chú thích mã nguồn rõ ràng, nghiên cứu kiến trúc hệ thống và cộng tác tốt trong các môi trường chuyên nghiệp[cite: 38].
              </p>
            </div>
          </div>
        </div>
